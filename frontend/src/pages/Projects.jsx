...
import api from '../api';                          // ← use api instance, not axios directly
        ...
        <select value={status} onChange={e => setStatus(e.target.value)} style={{ width: 'auto', minWidth: 120 }}>
          {statuses.map(s => (
            <option key={s} value={s}>
              {s === 'all' ? 'All Status' : s.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}
            </option>
          ))}
        </select>
        <button className="btn-primary" onClick={fetchProjects} style={{ padding: '12px 24px', fontSize: '0.8rem' }}>
          Search
        </button>
      </div>
