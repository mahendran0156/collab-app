      ...
      <div className="glass" style={{ padding:36 }}>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom:20 }}>
            <label style={{ display:'block', fontFamily:'Orbitron', fontSize:'0.7rem', color:'var(--text-muted)', marginBottom:8, letterSpacing:1 }}>USERNAME</label>
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <select value={form.field} onChange={e => setForm({...form, field:e.target.value})}>
              {fieldOptions.map(f => (
                <option key={f} value={f}>{f.replace('-',' ').replace(/\b\w/g, l => l.toUpperCase())}</option>
              ))}
            </select>
          </div>
          ...
          {/* Bio */}
          <div style={{ marginBottom:20 }}>
            <label style={{ display:'block', fontFamily:'Orbitron', fontSize:'0.7rem', color:'var(--text-muted)', marginBottom:8, letterSpacing:1 }}>BIO</label>
            <textarea value={form.bio} onChange={e => setForm({...form, bio:e.target.value})} placeholder="Tell collaborators about yourself..." style={{ minHeight:90, resize:'vertical' }} />
          </div>

          {/* Skills */}
          <div style={{ marginBottom:20 }}>
            <label style={{ display:'block', fontFamily:'Orbitron', fontSize:'0.7rem', color:'var(--text-muted)', marginBottom:8, letterSpacing:1 }}>SKILLS (comma separated)</label>
            <input type="text" value={form.skills} onChange={e => setForm({...form, skills:e.target.value})} placeholder="React, Node.js, UI Design..." />
          </div>

          {/* GitHub */}
          <div style={{ marginBottom:20 }}>
            <label style={{ display:'block', fontFamily:'Orbitron', fontSize:'0.7rem', color:'var(--text-muted)', marginBottom:8, letterSpacing:1 }}>GITHUB URL</label>
          ...
          <div style={{ marginBottom:28 }}>
            <label style={{ display:'block', fontFamily:'Orbitron', fontSize:'0.7rem', color:'var(--text-muted)', marginBottom:8, letterSpacing:1 }}>PORTFOLIO / WEBSITE</label>
            <input type="text" value={form.portfolio || form.website} onChange={e => setForm({...form, portfolio:e.target.value, website:e.target.value})} placeholder="https://yoursite.com" />
          </div>
