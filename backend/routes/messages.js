...
// ── GET /api/messages/:projectId — fetch chat history ────────────────────────
router.get('/:projectId', auth, async (req, res) => {
  try {
    const { projectId } = req.params;
    const { page = 1, limit = 50 } = req.query;

    // Only project members can read messages
    ...
    if (!isOwner && !isCollaborator) {
      return res.status(403).json({ error: 'Only project members can view messages' });
    }
    ...
    const [messages, total] = await Promise.all([
      Message.find({ project: projectId })
        ...
        .skip(skip)
        .limit(Number(limit))
        .populate('sender', 'username avatar field'),
...
router.post('/:projectId', auth, async (req, res) => {
  try {
    ...
    const isCollaborator = project.collaborators.map(String).includes(req.userId);
    if (!isOwner && !isCollaborator) {
      return res.status(403).json({ error: 'Only project members can send messages' });
    }
